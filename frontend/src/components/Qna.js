...
const QuestionList = () => {
    ...
    useEffect(() => {
        ...
        if (questionId) {
            fetch(`http://localhost:3000/api/questions/${questionId}/answers`)
                .then(response => response.json())
                .then(answersData => {
    ...
    const upvoteQuestion = (questionId) => {
        fetch(`http://localhost:3000/upvote/${questionId}`, { method: 'POST' })
            .then(response => response.json())
            .then(data => {
    ...
    const downvoteQuestion = (questionId) => {
        fetch(`http://localhost:3000/downvote/${questionId}`, { method: 'POST' })
            .then(response => response.json())
            .then(data => {
                console.log(data.message);
