...
app.get('/Qna/questions', (req, res) => {
    const sql = 'SELECT * FROM questionst';
...
app.post('/Qna/upvote/:id', (req, res) => {
    ...
    const sql = `UPDATE questionst SET upvotes = upvotes + 1 WHERE id = ${questionId}`;
    connection.query(sql, (err, results) => {
        if (err) {
...
app.post('/Qna/downvote/:id', (req, res) => {
    const questionId = req.params.id;
    const sql = `UPDATE questionst SET downvotes = downvotes + 1 WHERE id = ${questionId}`;
    connection.query(sql, (err, results) => {
        if (err) {
